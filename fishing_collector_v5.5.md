...
| `nearest_station` | 文字列 | 最近傍観測地点名（例：室戸）※気象DB (fishing_condition_db.csv) の地点名と一致させるため、短名を使用 |
...
※ 地点名・座標は `scripts/update-conditions.js` および `scripts/merge-data.js` の STATIONS 配列と完全一致させる。これにより `fishing_condition_db.csv` の「日付 × 地点名」キーで結合できる。
